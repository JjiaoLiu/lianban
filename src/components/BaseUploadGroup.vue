  ...
  <div>
    <div class="base-upload">
      <div v-for="(image,index) in imageList_sc" :index="index" class="label" v-if="!image.isDel">
        <div>
          <img :src="image.iamgeUrl" width="100%" height="100%" @click="outerVisible = true">
          <em @click="deleteI(image)" class="delete">×</em>
        </div>
      </div>
      <label for="upload-btn" class="label">
        <img src="../assets/img/icon_add_big.png" class="icon_add" style="width: 75%;height: 75%"/><span style="line-height: 60px"></span></label>
    ...
    <el-dialog :visible.sync="outerVisible">
      <el-carousel trigger="click">
        <el-carousel-item v-for="(image,index) in imageList_sc" :key="index" v-if="!image.isDel">
          <img :src="image.iamgeUrl" width="100%" height="100%">
        </el-carousel-item>
      </el-carousel>
    </el-dialog>
  </div>
      ...
      change($event) {
        ...
        this.$axios.post(process.env.API_HOST[3] + '/upload/image', formData).then(res => {
          if (res.data.success === true) {
            this.imageList_sc.push({
              iamgeUrl: res.data.result,
            });
