  ...
  <section>
    ...
    <el-form :inline="true" :model="formInline" class="filter-form" size="mini">
      ...
      <el-form-item label="房间名称">
        <el-input v-model.trim="formInline.roomName" placeholder="请输入房间名称"></el-input>
      </el-form-item>
    ...
    <el-table
      ...
      <el-table-column
        label="面积"
        width="140">
        <template slot-scope="scope">{{scope.row.acreage}}平米</template>
      </el-table-column>
      <el-table-column
        label="限制人数"
        prop="peopleNum"
...
<script>
  ...
  export default {
    name:"roomPart",components:{
      BaseHeader,BaseSpaceRoomSelect
    },data(){
      return {
        formInline:{
          meetingStatus:'',spaceId_sc:'',pageSize:15,page:1,enable:''
        },title:'添加',   //修改
        baseTitle:'房间',table:{},tableCellClassName:'tableCellClassName',tableHeaderClassName:'tableHeaderClassName',
      }
    },created(){
      // 请求列表数据
      this.getDataList();
      ...
      edit(item){
        this.$router.push('/home/roomPart/add?roomId_sc=' + item.roomId_sc)
      }, // 点击查询
      onSubmit(){
      ...
      onHeaderButton(){
        this.$router.push('/home/roomPart/add')
      }, // 点击某一页
      handleCurrentChange(){
        this.getDataList();
      }, // 获取列表
      getDataList(){
        this.$axios.post(process.env.API_HOST[1] + '/room/list',this.formInline).then(res => {
