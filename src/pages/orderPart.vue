<template>
  <section>
    ...
    <div class="total-box">
      <span>订单总数：<b>{{table.total}}</b>&nbsp;&nbsp;&nbsp;&nbsp;合计总数：{{table.orderAmount}}</span>
    </div>
    <el-table
      stripe
      :data="table.list"
    ...
    name:"orderPart",data(){
      return {
        orderAmount:"",formInline:{
          orderStatus:'',     // 空间状态是否有效
          timeType:1,pageSize:15,page:1,spaceId_sc:''
        },title:'添加',   //角色修改
        baseTitle:'房间/工位订单',table:{},tableCellClassName:'tableCellClassName',tableHeaderClassName:'tableHeaderClassName',
      }
    },components:{
      ...
      handleChangeDate(val){
        ...
        this.$router.push('/home/orderPart/deal?orderNo=' + item.orderNo)
      }, // 点击查询
      onSubmit(){
      ...
      getDataList(){
        this.$axios.post(process.env.API_HOST[1] + '/roomsorder/list',this.formInline).then(res => {
